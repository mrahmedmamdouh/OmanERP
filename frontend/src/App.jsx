...
export default function App(){
  ...
  function renderPage(){
    ...
    if(page==="dashboard"){
      ...
      return <div>
        <h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:"0 0 24px"}}>Dashboard</h2>
        <div style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:24}}>
          <SC icon={ic.dollar} label="Revenue" value={fmt(rev)} sub={"VAT Due: "+fmt(invs.reduce(function(s,i){return s+iVat(i);},0))} color={C.g}/>
          <SC icon={ic.users} label="Employees" value={emps.length} sub={om+" Omani"} color={C.b}/>
          <SC icon={ic.shield} label="Monthly SPF" value={fmt(spfT)} color={C.gld}/>
          <SC icon={ic.wallet} label="Expenses" value={fmt(exT)} color={C.acc}/>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          <div style={{background:C.card,border:"1px solid "+C.brd,borderRadius:14,padding:24}}>
            <h3 style={{fontSize:14,fontWeight:700,color:C.txt,marginBottom:16}}>Omanization</h3>
          ...
          <div style={{background:C.card,border:"1px solid "+C.brd,borderRadius:14,padding:24}}>
            <h3 style={{fontSize:14,fontWeight:700,color:C.txt,marginBottom:16}}>Recent Invoices</h3>
...
function InvPage(p){var invs=p.invs,reloadInvs=p.reloadInvs,toast=p.toast,co=p.co,showExport=p.showExport;
  ...
  if(viewI){var inv=viewI,sub=iTotal(inv),vat=iVat(inv),grand=iGrand(inv);
    return <div><div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}><Btn v="ghost" s="sm" onClick={function(){setViewI(null);}}>← Back</Btn><Btn v="secondary" s="sm" onClick={function(){window.print();}}><Ic d={ic.print} s={14}/> Print</Btn><Btn v="secondary" s="sm" onClick={function(){toast("Email sent");}}><Ic d={ic.send} s={14}/> Email</Btn>{inv.status==="pending"&&<Btn v="success" s="sm" onClick={function(){doStatus(inv,"paid");}}>Mark Paid</Btn>}{inv.status!=="cancelled"&&<Btn v="danger" s="sm" onClick={function(){doStatus(inv,"cancelled");}}>Cancel</Btn>}<Btn v="ghost" s="sm" onClick={function(){doDel(inv);}} style={{color:C.r}}><Ic d={ic.trash} s={14} c={C.r}/></Btn></div>
      <div style={{background:"#fff",color:"#111",borderRadius:14,padding:40,maxWidth:800,margin:"0 auto"}}><div style={{display:"flex",justifyContent:"space-between",borderBottom:"3px solid #c44530",paddingBottom:20,marginBottom:24}}><div><div style={{fontSize:24,fontWeight:800,color:"#c44530"}}>{co.nameAr}</div><div style={{fontSize:13,color:"#666"}}>{co.nameEn}</div><div style={{fontSize:11,color:"#888",marginTop:8}}>Tax ID: {co.taxId}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:20,fontWeight:700}}>فاتورة ضريبية</div><div style={{fontSize:12,color:"#666"}}>Tax Invoice</div><div style={{marginTop:12,fontSize:12,color:"#555",lineHeight:1.8}}><div>{inv.id}</div><div>{fD(inv.date)}</div></div></div></div>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",marginBottom:24,fontSize:13}}><thead><tr style={{background:"#f0f0f0"}}><th style={{padding:10,textAlign:"left"}}>Description</th><th style={{padding:10,width:60}}>Qty</th><th style={{padding:10,textAlign:"right",width:100}}>Price</th><th style={{padding:10,textAlign:"right",width:100}}>Total</th></tr></thead><tbody>{(inv.items||[]).map(function(item,i){return <tr key={i} style={{borderBottom:"1px solid #eee"}}><td style={{padding:10}}><div>{item.desc||item.descEn}</div><div style={{fontSize:11,color:"#888"}}>{item.descEn}</div></td><td style={{padding:10,textAlign:"center"}}>{item.qty}</td><td style={{padding:10,textAlign:"right"}}>{fmt(item.price)}</td><td style={{padding:10,textAlign:"right",fontWeight:600}}>{fmt(item.qty*item.price)}</td></tr>;})}</tbody></table>
        <div style={{display:"flex",justifyContent:"flex-end"}}><div style={{width:260}}><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",fontSize:13}}><span>Subtotal</span><span>{fmt(sub)}</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",fontSize:13}}><span>VAT 5%</span><span>{fmt(vat)}</span></div><div style={{display:"flex",justifyContent:"space-between",padding:"10px 0",fontSize:16,fontWeight:700,borderTop:"2px solid #c44530",marginTop:8}}><span>Total</span><span>{fmt(grand)}</span></div></div></div>
      </div></div>;}

  return <div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:0}}>Invoices</h2><div style={{display:"flex",gap:8}}><Btn v="secondary" s="sm" onClick={doExport}><Ic d={ic.dl} s={14}/> Export</Btn><Btn onClick={function(){setShowC(true);}}><Ic d={ic.plus} s={14} c="#fff"/> New Invoice</Btn></div></div>
    <div style={{display:"flex",gap:6,marginBottom:16}}>{["all","pending","paid","overdue","cancelled"].map(function(s){return <button key={s} type="button" onClick={function(){setFil(s);}} style={{padding:"6px 14px",borderRadius:20,border:"1px solid "+(fil===s?C.acc:C.brd),background:fil===s?C.accG:"transparent",color:fil===s?C.acc:C.mut,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>{s==="all"?"All":s}</button>;})}</div>
    <Modal open={showC} onClose={function(){setShowC(false);}} title="New Invoice" wide>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}><Inp label="Client (Arabic)" value={nI.client} onChange={function(v){setNI(Object.assign({},nI,{client:v}));}}/><Inp label="Client (English)" value={nI.clientEn} onChange={function(v){setNI(Object.assign({},nI,{clientEn:v}));}}/></div>
      {nI.items.map(function(item,idx){return <div key={idx} style={{display:"grid",gridTemplateColumns:"2fr 2fr 70px 100px",gap:8,marginBottom:6}}><Inp label={idx===0?"Desc (AR)":""} value={item.desc} onChange={function(v){var items=nI.items.slice();items[idx]=Object.assign({},items[idx],{desc:v});setNI(Object.assign({},nI,{items:items}));}}/><Inp label={idx===0?"Desc (EN)":""} value={item.descEn} onChange={function(v){var items=nI.items.slice();items[idx]=Object.assign({},items[idx],{descEn:v});setNI(Object.assign({},nI,{items:items}));}}/><Inp label={idx===0?"Qty":""} type="number" value={item.qty} onChange={function(v){var items=nI.items.slice();items[idx]=Object.assign({},items[idx],{qty:Number(v)});setNI(Object.assign({},nI,{items:items}));}}/><Inp label={idx===0?"Price":""} type="number" value={item.price} onChange={function(v){var items=nI.items.slice();items[idx]=Object.assign({},items[idx],{price:Number(v)});setNI(Object.assign({},nI,{items:items}));}}/></div>;})}
      <div style={{display:"flex",gap:8,marginTop:14}}><Btn v="secondary" s="sm" onClick={function(){setNI(Object.assign({},nI,{items:nI.items.concat([{desc:"",descEn:"",qty:1,price:0}])}));}}>+ Add Item</Btn><div style={{flex:1}}/><Btn v="ghost" onClick={function(){setShowC(false);}}>Cancel</Btn><Btn onClick={doCreate} disabled={busy}>{busy?"Creating...":"Create"}</Btn></div>
    </Modal>
    <Tbl cols={[{key:"id",label:"Invoice #",render:function(r){return <span style={{fontWeight:600,color:C.acc}}>{r.id}</span>;}},{key:"client",label:"Client",render:function(r){return r.clientEn;}},{key:"date",label:"Date",render:function(r){return fD(r.date);}},{key:"total",label:"Total",render:function(r){return <strong>{fmt(iGrand(r))}</strong>;}},{key:"status",label:"Status",render:function(r){return <Badge color={r.status==="paid"?"g":r.status==="pending"?"y":"r"}>{r.status}</Badge>;}},{key:"act",label:"",render:function(r){return <div style={{display:"flex",gap:4}}><Btn v="ghost" s="sm" onClick={function(e){e.stopPropagation();setViewI(r);}}><Ic d={ic.eye} s={14}/></Btn>{r.status==="pending"&&<Btn v="ghost" s="sm" onClick={function(e){e.stopPropagation();doStatus(r,"paid");}}><Ic d={ic.check} s={14} c={C.g}/></Btn>}<Btn v="ghost" s="sm" onClick={function(e){e.stopPropagation();doDel(r);}}><Ic d={ic.trash} s={14} c={C.r}/></Btn></div>;}}]} data={fd} onRow={function(r){setViewI(r);}}/></div>;}

function EmpPage(p){var emps=p.emps,reloadEmps=p.reloadEmps,toast=p.toast,showExport=p.showExport;
  ...
  return <div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:0}}>Employees ({emps.length})</h2><div style={{display:"flex",gap:8}}><Btn v="secondary" s="sm" onClick={doExport}><Ic d={ic.dl} s={14}/> Export</Btn><Btn onClick={function(){setForm(empty);setEditE(null);setShowF(true);}}><Ic d={ic.plus} s={14} c="#fff"/> Add Employee</Btn></div></div>
    <Modal open={showF} onClose={function(){setShowF(false);}} title={editE?"Edit Employee":"Add Employee"} wide>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}><Inp label="Name (AR)" value={form.name} onChange={function(v){setForm(Object.assign({},form,{name:v}));}}/><Inp label="Name (EN)" value={form.nameEn} onChange={function(v){setForm(Object.assign({},form,{nameEn:v}));}}/><Sel label="Nationality" value={form.nat} onChange={function(v){setForm(Object.assign({},form,{nat:v}));}} options={[{v:"Omani",l:"Omani"},{v:"Indian",l:"Indian"},{v:"Pakistani",l:"Pakistani"},{v:"Egyptian",l:"Egyptian"},{v:"Other",l:"Other"}]}/><Sel label="Department" value={form.dept} onChange={function(v){setForm(Object.assign({},form,{dept:v}));}} options={DEPTS.map(function(d){return{v:d,l:d};})}/><Inp label="Role" value={form.roleEn} onChange={function(v){setForm(Object.assign({},form,{roleEn:v}));}}/><Inp label="Salary (OMR)" type="number" value={form.salary} onChange={function(v){setForm(Object.assign({},form,{salary:Number(v)}));}}/><Inp label="Allowances" type="number" value={form.allow} onChange={function(v){setForm(Object.assign({},form,{allow:Number(v)}));}}/><Inp label="Email" value={form.email} onChange={function(v){setForm(Object.assign({},form,{email:v}));}}/></div>
      <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}}><Btn v="ghost" onClick={function(){setShowF(false);}}>Cancel</Btn><Btn onClick={doSave} disabled={busy}>{busy?"Saving...":"Save"}</Btn></div>
    </Modal>
    <Modal open={!!delE} onClose={function(){setDelE(null);}} title="Confirm Delete"><p style={{color:C.mut,marginBottom:20}}>{delE?delE.nameEn:""}</p><div style={{display:"flex",justifyContent:"flex-end",gap:8}}><Btn v="ghost" onClick={function(){setDelE(null);}}>Cancel</Btn><Btn v="danger" onClick={doDelete} disabled={busy}>{busy?"Deleting...":"Delete"}</Btn></div></Modal>
...
function ExpPage(p){var exps=p.exps,reloadExps=p.reloadExps,toast=p.toast,showExport=p.showExport;
  var [showA,setShowA]=useState(false);var [busy,setBusy]=useState(false);var cats=["rent","utilities","supplies","insurance","other"];var [form,setForm]=useState({descEn:"",amount:0,cat:"utilities",vendor:"",vatI:false});
  ...
  return <div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:0}}>Expenses</h2><div style={{display:"flex",gap:8}}><Btn v="secondary" s="sm" onClick={doExport}><Ic d={ic.dl} s={14}/></Btn><Btn onClick={function(){setShowA(true);}}><Ic d={ic.plus} s={14} c="#fff"/> Add</Btn></div></div>
    <div style={{display:"flex",gap:14,marginBottom:24}}><SC icon={ic.wallet} label="Total" value={fmt(totalExp)} color={C.acc}/><SC icon={ic.dollar} label="Input VAT" value={fmt(inputVAT)} color={C.b}/></div>
    <Modal open={showA} onClose={function(){setShowA(false);}} title="Add Expense"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Inp label="Description" value={form.descEn} onChange={function(v){setForm(Object.assign({},form,{descEn:v}));}}/><Inp label="Amount" type="number" value={form.amount} onChange={function(v){setForm(Object.assign({},form,{amount:v}));}}/><Sel label="Category" value={form.cat} onChange={function(v){setForm(Object.assign({},form,{cat:v}));}} options={cats.map(function(c){return{v:c,l:c};})}/><Inp label="Vendor" value={form.vendor} onChange={function(v){setForm(Object.assign({},form,{vendor:v}));}}/></div><label style={{display:"flex",alignItems:"center",gap:8,marginTop:12,fontSize:13,color:C.txt,cursor:"pointer"}}><input type="checkbox" checked={form.vatI} onChange={function(e){setForm(Object.assign({},form,{vatI:e.target.checked}));}}/>VAT Included</label><div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}}><Btn v="ghost" onClick={function(){setShowA(false);}}>Cancel</Btn><Btn onClick={doAdd} disabled={busy}>{busy?"Adding...":"Save"}</Btn></div></Modal>
...
function OmanPage(p){var emps=p.emps,toast=p.toast,showExport=p.showExport;
  ...
  return <div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:0}}>Omanization</h2><Btn v="secondary" onClick={doExport}><Ic d={ic.dl} s={14}/> Export</Btn></div>
    <div style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:24}}><SC icon={ic.flag} label="Overall" value={(emps.length>0?Math.round(totalO/emps.length*100):0)+"%"} sub={totalO+"/"+emps.length} color={C.acc}/><SC icon={ic.check} label="Compliant" value={ok} color={C.g}/><SC icon={ic.alert} label="Non-Compliant" value={depts.length-ok} color={C.r}/></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:14}}>{depts.map(function(d){var met=d.pct>=d.target,gap=Math.max(0,Math.ceil(d.target/100*d.t)-d.o);return <div key={d.dept} style={{background:C.card,border:"1px solid "+(met?C.gD:C.rD),borderRadius:14,padding:18,position:"relative",overflow:"hidden"}}><div style={{position:"absolute",top:0,right:0,padding:"3px 10px",borderRadius:"0 14px 0 10px",background:met?C.gB:C.rB,fontSize:10,fontWeight:700,color:met?C.g:C.r}}>{met?"Compliant":"Action Needed"}</div><h4 style={{fontSize:14,fontWeight:700,color:C.txt,marginBottom:10}}>{d.dept}</h4><div style={{display:"flex",justifyContent:"space-between",marginBottom:6,fontSize:12}}><span style={{color:C.mut}}>Current: <strong style={{color:C.txt}}>{d.pct}%</strong></span><span style={{color:C.mut}}>Target: <strong style={{color:met?C.g:C.r}}>{d.target}%</strong></span></div><PBar value={d.pct} max={d.target} color={met?C.g:C.r} h={10}/>{!met&&<div style={{marginTop:10,padding:"7px 10px",background:C.rB,borderRadius:8,border:"1px solid "+C.rD,fontSize:11,color:C.r}}>⚠ Need {gap} more Omanis</div>}</div>;})}</div></div>;}

function RepPage(p){var invs=p.invs,emps=p.emps,exps=p.exps,toast=p.toast,showExport=p.showExport;
  ...
  function doExport(){showExport("pnl.csv",buildCSV(["Item","Amount"],[["Revenue",inc.toFixed(3)],["Payroll",pay.toFixed(3)],["SPF ER",spfC.toFixed(3)],["Expenses",expA.toFixed(3)],["Net Profit",np.toFixed(3)]]),"csv");}
  return <div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:0}}>Reports</h2><Btn onClick={doExport}><Ic d={ic.dl} s={14} c="#fff"/> Export P&L</Btn></div>
    <div style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:24}}><SC icon={ic.chart} label="Income" value={fmt(inc)} color={C.g}/><SC icon={ic.wallet} label="Costs" value={fmt(costs)} color={C.r}/><SC icon={ic.dollar} label="Profit" value={fmt(np)} color={np>=0?C.g:C.r}/><SC icon={ic.pie} label="Margin" value={mg+"%"} color={C.gld}/></div>
    <div style={{background:C.card,border:"1px solid "+C.brd,borderRadius:14,padding:24}}><h3 style={{fontSize:15,fontWeight:700,color:C.txt,marginBottom:16}}>Income Statement</h3>
      {[{l:"Revenue (Paid)",v:inc,c:C.g,b:true},{l:"— Payroll",v:-pay,c:C.r},{l:"— SPF Employer",v:-spfC,c:C.r},{l:"— Expenses",v:-expA,c:C.r},{l:"Net Profit",v:np,c:np>=0?C.g:C.r,b:true,a:true}].map(function(r,i){return <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"12px 14px",background:r.a?C.accG:i%2===0?C.hov:"transparent",borderRadius:6,marginBottom:2,borderTop:r.a?"2px solid "+C.acc:"none",marginTop:r.a?8:0}}><span style={{fontSize:13,color:C.txt,fontWeight:r.b?700:400}}>{r.l}</span><span style={{fontSize:r.a?18:14,fontWeight:r.b?800:600,color:r.c}}>{r.v<0?"−":""}{fmt(Math.abs(r.v))}</span></div>;})}</div></div>;}

function SetPage(p){var co=p.co,setCo=p.setCo,toast=p.toast,showExport=p.showExport;var [form,setForm]=useState(co);var [busy,setBusy]=useState(false);
  ...
  return <div><h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:"0 0 20px"}}>Settings</h2>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
      <div style={{background:C.card,border:"1px solid "+C.brd,borderRadius:14,padding:24}}><h3 style={{fontSize:15,fontWeight:700,color:C.txt,marginBottom:16}}>Company Info</h3><div style={{display:"flex",flexDirection:"column",gap:12}}><Inp label="Company (AR)" value={form.nameAr} onChange={function(v){setForm(Object.assign({},form,{nameAr:v}));}}/><Inp label="Company (EN)" value={form.nameEn} onChange={function(v){setForm(Object.assign({},form,{nameEn:v}));}}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Inp label="CR No." value={form.cr} onChange={function(v){setForm(Object.assign({},form,{cr:v}));}}/><Inp label="Tax ID" value={form.taxId} onChange={function(v){setForm(Object.assign({},form,{taxId:v}));}}/></div><Inp label="Address" value={form.address} onChange={function(v){setForm(Object.assign({},form,{address:v}));}}/><Inp label="Email" value={form.email} onChange={function(v){setForm(Object.assign({},form,{email:v}));}}/></div><Btn onClick={doSave} disabled={busy} style={{marginTop:18}}>{busy?"Saving...":"Save Settings"}</Btn></div>
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        <div style={{background:C.card,border:"1px solid "+C.brd,borderRadius:14,padding:24}}><h3 style={{fontSize:15,fontWeight:700,color:C.txt,marginBottom:16}}>Data Export</h3><div style={{display:"flex",flexDirection:"column",gap:8}}>
          <Btn v="secondary" onClick={function(){showExport("erp-backup.json",buildJSON({employees:p.emps,invoices:p.invs,expenses:p.exps,settings:form}),"json");}} style={{width:"100%"}}><Ic d={ic.dl} s={14}/> Full Backup (JSON)</Btn>
          <Btn v="secondary" onClick={function(){showExport("employees.csv",buildCSV(["Name","Dept","Salary","Nationality"],p.emps.map(function(e){return[e.nameEn,e.dept,e.salary,e.nat];})),"csv");}} style={{width:"100%"}}><Ic d={ic.users} s={14}/> Employees CSV</Btn>
          <Btn v="secondary" onClick={function(){showExport("invoices.csv",buildCSV(["ID","Client","Total","Status"],p.invs.map(function(i){return[i.id,i.clientEn,iGrand(i).toFixed(3),i.status];})),"csv");}} style={{width:"100%"}}><Ic d={ic.file} s={14}/> Invoices CSV</Btn>
        </div></div>
      </div>
    </div></div>;}
