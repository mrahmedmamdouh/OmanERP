...
export default function App(){
  ...
  var navLabels={dashboard:t("dashboard"),invoices:t("invoices"),employees:t("employees"),payroll:t("payrollMod"),expenses:t("expenses"),vat:t("vatComp"),spf:t("spf"),omanization:t("oman"),reports:t("reports"),settings:t("settings")};
  ...
  function renderPage(){
    ...
    if(page==="invoices") return <InvPage invs={invs} reloadInvs={reloadInvs} t={t} toast={toast} co={co} showExport={showExport}/>;
...
function InvPage(p){var invs=p.invs,reloadInvs=p.reloadInvs,t=p.t,toast=p.toast,co=p.co,showExport=p.showExport;
  ...
  return <div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:0}}>{(p.)?t("invoices")}</h2><div style={{display:"flex",gap:8}}><Btn v="secondary" s="sm" onClick={doExport}><Ic d={ic.dl} s={14}/> {t("export")}</Btn><Btn onClick={function(){setShowC(true);}}><Ic d={ic.plus} s={14} c="#fff"/> {t("newInv")}</Btn></div></div>
    ...
    <Modal open={showC} onClose={function(){setShowC(false);}} title=t("newInv") wide>
      ...
      <div style={{display:"flex",gap:8,marginTop:14}}><Btn v="secondary" s="sm" onClick={function(){setNI(Object.assign({},nI,{items:nI.items.concat([{desc:"",descEn:"",qty:1,price:0}])}));}}>{t("addItem")}</Btn><div style={{flex:1}}/><Btn v="ghost" onClick={function(){setShowC(false);}}>{t("cancel")}</Btn><Btn onClick={doCreate} disabled={busy}>{(p.)?busy?t("creating"):t("create")}</Btn></div>
    </Modal>
...
function EmpPage(p){var emps=p.emps,reloadEmps=p.reloadEmps,toast=p.toast,showExport=p.showExport;
  ...
  return <div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:0}}>{t("employees")} ({emps.length})</h2><div style={{display:"flex",gap:8}}><Btn v="secondary" s="sm" onClick={doExport}><Ic d={ic.dl} s={14}/> {t("export")}</Btn><Btn onClick={function(){setForm(empty);setEditE(null);setShowF(true);}}><Ic d={ic.plus} s={14} c="#fff"/> {t("addEmp")}</Btn></div></div>
...
function ExpPage(p){var exps=p.exps,reloadExps=p.reloadExps,toast=p.toast,showExport=p.showExport;
  ...
  return <div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{fontSize:22,fontWeight:700,color:C.txt,margin:0}}>{t("expenses")}</h2><div style={{display:"flex",gap:8}}><Btn v="secondary" s="sm" onClick={doExport}><Ic d={ic.dl} s={14}/></Btn><Btn onClick={function(){setShowA(true);}}><Ic d={ic.plus} s={14} c="#fff"/> {t("addExp")}</Btn></div></div>
